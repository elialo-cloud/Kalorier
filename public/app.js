...
let goals={
  ...defaultGoals,
  ...JSON.parse(localStorage.getItem('kalorier-goals')||'{}')
};
...
function ensureUI(){
  ...
  if(
    ...
    wrap.innerHTML=`
      <button class="ghost" id="todayTop">
        Idag
      </button>
  ...
  if(!$('#suggestions')){
    ...
    s.innerHTML=`
      <div class="section-head">
        <h2>För att nå ditt mål</h2>
        <span id="suggestionMeta"></span>
      </div>
    ...
    s.innerHTML=`
      ...
      <div class="settings-body">
        ...
        <div
          ...
          <button data-goal="lose">
            Gå ner
          </button>

          <button data-goal="maintain">
            Behålla
          </button>

          <button data-goal="gain">
            Gå upp
          </button>
        ...
        <p class="settings-note">
          Du bestämmer själv målen.
          De används för progress och förslag.
        </p>
...
function renderSuggestions(t){
  ...
  if(
    ...
    picks.push([
      '🍗',
      'Kyckling + ris',
      'mycket protein, lätt att portionera'
    ]);
  }
  ...
  if(missing.carbs>35){
    ...
    picks.push([
      '🍚',
      'Ris eller potatis',
      'enkelt sätt att fylla på kolhydrater'
    ]);
  }
  ...
  if(missing.fat>15){
    ...
    picks.push([
      '🥑',
      'Avokado eller nötter',
      'bra sätt att få upp fett'
    ]);
  }
...
async function loadDay(){
  ...
  try{
    ...
    const r=await fetch(
      `/api/diary?date=${encodeURIComponent(currentDate)}`
    );

    if(!r.ok){
      throw new Error('Diary error');
    }
          ...
          return `
            <button
              ...
              <div class="result-top">
                ...
                <span
                  class="source-badge source-${sourceClass}">
                  ${label}
                </span>
...
function aliasTerms(query){
  ...
  for(
    ...
    of Object.entries(SEARCH_ALIASES)
  ){
...
function aiCandidate(query){
  ...
  for(
    const food of AI_FOODS
  ){

    for(
      const match of food.match
    ){
      ...
      if(qt.length){
        ...
        for(
          ...
          const found=mt.some(x=>
            x===token ||
            x.startsWith(token) ||
            similarity(token,x)>=0.75
          );
...
async function searchAll(q=''){
  ...
  /*
    ...
    ska ett gammalt svar inte kunna
    ersätta det senaste.
  */
  ...
  try{
    ...
    */
    ...
    /*
      AI ska inte lägga sig över en bra
      riktig databasprodukt.

      Men om databasen inte hittar något
      riktigt bra kan AI visas först.
    */

    const aiUseful=
      ai &&
      bestDatabaseScore<2500;
    ...
    if(!final.length){
      ...
      $('#searchHint').textContent=
        'AI-förslag + träffar från databaserna.';
    ...
    if(ai){
      ...
      $('#searchHint').textContent=
        'AI-förslag – databaserna kunde inte läsas.';
