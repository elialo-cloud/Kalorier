// Kalorier V4.2: stability, search UX, goals, and source badge polish.
(() => {
  // app.js expects this optional hook to exist. A missing element previously stopped
  // all remaining initialization, including search, scanner, and settings buttons.
  ...
  function saveGoalsSafely(){
    ...
    const saved={
      kcal:num('targetKcal',2200,800), protein:num('targetProtein',160,0),
      carbs:num('targetCarbs',220,0), fat:num('targetFat',70,0),
      goal:document.querySelector('#goalPills button.active')?.dataset.goal||'maintain'
    };
  ...
  // Event delegation survives dynamically-created settings UI.
  ...
  // Search normalization: trailing/duplicate whitespace must never clear a valid query.
  ...
  function badgeFor(row){
    ...
    return isOff
      ? '<span class="source-badge off" title="Open Food Facts" aria-label="Open Food Facts"><span class="off-mark">●</span> OFF</span>'
      : '<span class="source-badge liv" title="Livsmedelsverket" aria-label="Livsmedelsverket">LIVS</span>';
  }
  function applyBadges(){
    document.querySelectorAll('.result,.food').forEach(row=>{
      const title=row.querySelector('strong');
      if(!title || title.querySelector('.source-badge')) return;
      title.insertAdjacentHTML('beforeend',' '+badgeFor(row));
    });
  }
  ...
  style.textContent=`
    ...
    .source-badge .off-mark{font-size:7px;line-height:1}
    .source-badge.liv,.source-badge.off{background:#f2f6e9;color:#566a2c;border:1px solid #dce6c9}
  `;
  document.head.appendChild(style);
  new MutationObserver(applyBadges).observe(document.documentElement,{childList:true,subtree:true});
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',()=>{setVersion();bindSearch();applyBadges()});
  else applyBadges();
})();
